// used reference:  GitHub repository https://github.com/Jurinho/2d-Histogram-filter-c-/blob/master/helpers.cpp
...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++) {
			float grid_values = grid[i][j] /sum_values;
			newGrid[i][j] = grid_values;
		}
	}
	return newGrid;
}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
 ...
 int width = grid[0].size();
    ...
    vector < vector <float> > newGrid (height,  vector <float>(width));
	const float main_prob = 1.0 - blurring;
	const float diagonal_prob = blurring / 12.0;
	const float orthogonal_prob = blurring / 6.0;

  	vector < vector <float> >  window {{diagonal_prob, orthogonal_prob, diagonal_prob},
					{orthogonal_prob, main_prob, orthogonal_prob},
					{diagonal_prob, orthogonal_prob, diagonal_prob}
					};

  	for (int i = 0; i < height; i++){
		for (int j = 0; j < width; j++) {
			float grid_values = grid[i][j];
			for (int dx = -1; dx <= 1; dx+=1) 
			{
				for (int dy = -1; dy <= 1; dy+=1)
				 {
					int new_i_value = (i + dy + height) % height;
					int new_j_value = (j + dx + width) % width;
					float calculated = window[dx+1][dy+1];
					newGrid[new_i_value][new_j_value] += grid_values * calculated;
				}
			}
		}
	}
